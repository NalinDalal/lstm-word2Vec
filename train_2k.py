"""Day 4: Train at 2k steps with gradient analysis"""
...
data = create_sequences(text, char_to_idx, 40)
...
for step in range(2000):
    ...
    optimizer.zero_grad()
    h0 = torch.zeros(1, len(batch), 128)
    c0 = torch.zeros(1, len(batch), 128)
...
print("\n=== GRADIENT ANALYSIS ===")
print(f"Grad norm recordings: {len(grad_norms)}")
print(
...
print(f"Steps with grad < 0.1 (vanishing): {vanishing}")
print(f"Steps with grad > 5.0 (exploding): {exploding}")
